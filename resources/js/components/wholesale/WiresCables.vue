<template name="wires-cables">
  <v-container class="shrink-width wires-cables-container text-center">
    <v-row>
      <v-col cols="12">
        <h3 class="wholesale-subcat-title mb-5">Wires and <span>cables categories</span></h3>
      </v-col>
      <v-col class="pb-11" lg="4" md="4" sm="12" cols="12">
        <a href="#wires" class="round-block">
          <div class="round-block__img">
            <v-img max-width="280px" src="/images/wires-cables/wires/wire1.jpg" alt=""></v-img>
          </div>
          <div class="round-block__content">
            <h2>Wires</h2>
          </div>
        </a>
      </v-col>
      <v-col class="pb-11" lg="4" md="4" sm="12" cols="12">
        <a href="#cables" class="round-block">
          <div class="round-block__img">
            <v-img max-width="280px" src="/images/wires-cables/cables/cable1.jpg" alt=""></v-img>
          </div>
          <div class="round-block__content">
            <h2>Cables</h2>
          </div>
        </a>
      </v-col>
      <v-col class="pb-11" lg="4" md="6" sm="12" cols="12">
        <a href="#readyMadeCables" class="round-block">
          <div class="round-block__img">
            <v-img max-width="280px" src="/images/wires-cables/ready-made-cables/ready-made-cables-thumb.jpg"
                   alt=""></v-img>
          </div>
          <div class="round-block__content">
            <h2>Ready-made cables/Cables assemblies</h2>
          </div>
        </a>
      </v-col>
    </v-row>

    <v-divider class="pb-10"></v-divider>

    <v-row id="wires">
      <v-col cols="12">
        <h2 class="subcat-title mb-5">Wires</h2>
      </v-col>
      <v-col lg="8" md="8" sm="12" cols="12">
        <v-img class="subcat-img" max-width="600px" max-height="400px" src="/images/wires-cables/wires/wire1.jpg"
               alt=""></v-img>
      </v-col>
      <v-col lg="4" md="4" sm="12" cols="12" class="text-left">
        <h3>LIY wires <span>for electronics</span></h3>
        <p class="mb-0">Vast range</p>
        <p class="mb-0">Help with making the right choice</p>
        <p class="mb-0">Special offer - measuring cables to required length</p>
      </v-col>
    </v-row>
    <v-row class="wholesale-subcat-info">
      <v-col cols="12">
        <h3 class="mt-5">
          Liy
        </h3>
      </v-col>
      <v-col cols="12" class="text-left">
        <h4 class="mb-2">
          Conductors with various cross-sections
        </h4>
        <ul class="pl-0">
          <li>
            Conductors with different cross-sectional areas for different uses
          </li>
          <li>
            0,14mm2; 0,25mm2; 0,34mm2........ 1mm2; 1,5mm2; 2,5mm2;..........10mm2 or 28AWG, 24AWG, 22AWG…….18AWG,
            16AWG, 14AWG...
          </li>
        </ul>
      </v-col>
      <v-col cols="12" class="text-left">
        <h4 class="mb-2">Colored wires</h4>
        <ul class="pl-0">
          <li>
            A wide specter of colors and shades
          </li>
          <li>
            Two-colored wires with vertical stripe or ring stripe
          </li>
        </ul>
      </v-col>
      <v-col cols="12" class="text-left">
        <h4 class="mb-2">
          Standard
        </h4>
        <ul class="pl-0">
          <li>
            Conductors manufactured by standards for European and American markets (VDE, UL, cUL...)
          </li>
          <li>
            Conductors of many different structures and materials, tested for
            functioning in out-of-ordinary circumstances (high temperature, moisture, indoor/outdoor use...)
          </li>
        </ul>
        <p class="mt-5">*for certain conductors </p>
      </v-col>
    </v-row>
    <base-miny-gallery :minyGallery="true" :className="'wire'" :galleryImages="wireImages"></base-miny-gallery>

    <v-divider class="pb-10"></v-divider>

    <v-row id="cables" daTa-aos="fade-down" data-aos-delay="400" data-aos-easing="ease-in-out">
      <v-col cols="12">
        <h2 class="subcat-title mb-5">Cables</h2>
      </v-col>
      <v-col lg="8" md="8" sm="12" cols="12">
        <v-img class="subcat-img" max-width="600px" max-height="400px" src="/images/wires-cables/cables/cable2.jpg"
               alt=""></v-img>
      </v-col>
      <v-col lg="4" md="4" sm="12" cols="12" class="text-left">
        <h3>Multi-core cables to <span>fit all your needs</span></h3>
        <p class="mb-0">Large choice</p>
        <p class="mb-0">Help with choosing the right cable</p>
        <p class="mb-0">Special-offer - measuring cables to required length</p>
      </v-col>
    </v-row>
    <v-row class="wholesale-subcat-info" daTa-aos="fade-down" data-aos-delay="400" data-aos-easing="ease-in-out">
      <v-col cols="12">
        <h3 class="mt-5">
          Multi-core LIYY, LIY-CY, signal, control and power cables
        </h3>
      </v-col>
      <v-col cols="12" class="text-left">
        <h4 class="mb-2">
          Multi-core cables
        </h4>
        <ul class="pl-0">
          <li>
            Cables with two or more cores with different cross-sections and colors
          </li>
          <li>
            We offer multi-core cables with all configurations - 2x0,25mm2 ; 2x0,34mm2… 5x0,25mm2, 5x0,34mm2…..
            the same goes for UL certified cables in
            which case the configurations are marked in the following way - 2x24AWG; 2x22AWG….. 5x24AWG; 5x22AWG etc.
          </li>
        </ul>
      </v-col>
      <v-col cols="12" class="text-left">
        <h4 class="mb-2">Power cables</h4>
        <ul class="pl-0">
          <li>
            All kinds of power cables for different currents
          </li>
        </ul>
        <p class="mt-5">*for certain conductors </p>
      </v-col>
    </v-row>
    <base-miny-gallery :minyGallery="true" :className="'cable'" :galleryImages="cableImages"></base-miny-gallery>
    <v-divider class="pb-10"></v-divider>

    <v-row id="readyMadeCables" daTa-aos="fade-down" data-aos-delay="400" data-aos-easing="ease-in-out">
      <v-col cols="12">
        <h2 class="subcat-title mb-5">
          Ready-made cable assemblies that you need all the time
        </h2>
      </v-col>
      <v-col cols="12" class="text-left">
        <v-row class="wholesale-subcat-info" daTa-aos="fade-down" data-aos-delay="400" data-aos-easing="ease-in-out">
          <v-col lg="8" md="8" sm="12" cols="12">
            <v-img class="subcat-img" max-width="600px" max-height="400px"
                   src="/images/wires-cables/ready-made-cables/ready-made-cables.jpg" alt=""></v-img>
          </v-col>
          <v-col lg="4" md="4" sm="12" cols="12" class="text-left">
            <h3>Cable <span>assemblies</span></h3>
            <p class="mb-0">Wide assortment of cable assemblies</p>
            <p class="mb-0">Stay connected at any moment</p>
            <p class="mb-0">Possibility of special order</p>
          </v-col>
          <v-col cols="12">
            <h4 class="mb-2">
              USB cables
            </h4>
            <ul class="pl-0">
              <li>
                We offer USB cables of different colors and lengths, with the connectors combinations that work for you.
              </li>
              <li>
                Combinations:
                <p class="mb-0 ml-5">- Type A male – Type A female (extension USB type A cable with or without screws
                  for mounting</p>
                <p class="mb-0 ml-5">- Type A – Type B, Type B mini, Type B micro</p>
                <p class="mb-0 ml-5">- Type A – Type</p>
                <p class="mb-0 ml-5">- and others... </p>
              </li>
              <li>
                Angled connectors (90 degrees, left angle, right angle, etc…)
              </li>
            </ul>
          </v-col>
          <v-col cols="12">
            <h4 class="mb-2">
              Audio and video cables
            </h4>
            <ul class="pl-0">
              <li>
                Monitor cables for all of your screen types (HDMI, DVI, DP, VGA…)
              </li>
              <li>
                Audio cables (with speaker jacks and other connectors)
              </li>
            </ul>
          </v-col>
          <v-col cols="12">
            <h4 class="mb-2">
              And others per your request...
            </h4>
          </v-col>
        </v-row>
        <v-row class="justify-center mt-5" daTa-aos="fade-down" data-aos-delay="400" data-aos-easing="ease-in-out">
          <v-col class="text-center" lg="6" md="6" sm="8" cols="8">
            <p>
              We also offer wire and cable harness manufacture per your design - from smaller cable harness to bigger
              and more complex cable systems.
            </p>
            <p class="wholesale-click-picture">
              Click the <span>picture for more details</span>
            </p>
            <router-link to="/wires-and-harness" class="round-block">
              <div class="round-block__img">
                <v-img max-width="200px" max-height="180px" src="/images/wires-cables/ready-made-cable.jpg"
                       alt=""></v-img>
              </div>
            </router-link>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
    <base-miny-gallery :minyGallery="true" :className="'readyMadeCable'" :galleryImages="readyMadeCableImages"></base-miny-gallery>
  </v-container>
</template>
<script>

  import axios from 'axios'

  export default {
    name: 'WiresCables',
    data() {
      return {
        wireImages: [],
        cableImages: [],
        readyMadeCableImages: [],
        className: null
      }
    },
    mounted() {
      axios.get('/api/index', {
        params: {
          folder: 'wires-cables/wires'
        }
      })
        .then((response) => {
          let images = response.data.images;

          images.forEach(image => {
            this.wireImages.push('/images/wires-cables/wires/' + image);
          });
        })
        .catch(error => console.log(error))

      axios.get('/api/index', {
        params: {
          folder: 'wires-cables/cables'
        }
      })
        .then((response) => {
          let images = response.data.images;

          images.forEach(image => {
            this.cableImages.push('/images/wires-cables/cables/' + image);
          });
        })
        .catch(error => console.log(error))

      axios.get('/api/index', {
        params: {
          folder: 'wires-cables/ready-made-cables'
        }
      })
        .then((response) => {
          let images = response.data.images;

          images.forEach(image => {
            this.readyMadeCableImages.push('/images/wires-cables/ready-made-cables/' + image);
          });
        })
        .catch(error => console.log(error))
    },
  }
</script>
